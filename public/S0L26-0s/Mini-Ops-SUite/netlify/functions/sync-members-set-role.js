...
exports.handler = async (event) => {
  ...
  if(!targetUserId || !ROLES.includes(role)) return bad(event, 400, 'bad-input');

  try{
    const out = await tx(async (client) => {
      const t = await client.query('SELECT id, role FROM sync_users WHERE id=$1 AND org_id=$2 FOR UPDATE', [targetUserId, tokenUser.orgId]);
      if(t.rowCount !== 1) return { err: { status: 404, msg: 'user-not-found' } };

      const curRole = t.rows[0].role;
      if(curRole === 'owner' && role !== 'owner'){
        const owners = await client.query("SELECT count(*)::int AS c FROM sync_users WHERE org_id=$1 AND role='owner'", [tokenUser.orgId]);
        if(owners.rows[0].c <= 1) return { err: { status: 409, msg: 'cannot-remove-last-owner' } };
      }

      await client.query('UPDATE sync_users SET role=$1 WHERE id=$2 AND org_id=$3', [role, targetUserId, tokenUser.orgId]);
      await auditTx(client, event, { orgId: tokenUser.orgId, userId: tokenUser.sub, deviceId: tokenUser.did||null, action:'member.setRole', severity:'info', detail:{ targetUserId, role } });
      return { ok: true };
    });
