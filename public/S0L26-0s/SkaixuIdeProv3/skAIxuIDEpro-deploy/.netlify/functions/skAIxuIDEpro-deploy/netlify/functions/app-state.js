...
module.exports = __toCommonJS(app_state_exports);
...
async function upsertAppState(sql, memberId, appKey, snapshot = {}) {
...
async function readAppState(sql, memberId, appKey) {
  const safeKey = String(appKey || "app").slice(0, 240);
  const rows = await sql`
