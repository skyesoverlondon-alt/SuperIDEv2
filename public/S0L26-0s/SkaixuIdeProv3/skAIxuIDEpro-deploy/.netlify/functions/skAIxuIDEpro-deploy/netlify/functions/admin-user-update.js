...
var admin_user_update_exports = {};
__export(admin_user_update_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(admin_user_update_exports);
...
var handler = async (event, context) => {
  ...
  try {
    ...
    const role = ["admin", "member"].includes(body.role) ? body.role : void 0;
    const status = ["active", "suspended", "disabled"].includes(body.status) ? body.status : void 0;
    const requestCap = clampInt(body.monthly_request_cap, 250, 1, 1e6);
    const tokenCap = clampInt(body.monthly_token_cap, 25e4, 1e3, 1e9);
    const rows = await sql`
      UPDATE kaixu_members
