...
var gateway_stream_exports = {};
__export(gateway_stream_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(gateway_stream_exports);

// netlify/functions/_lib/kaixu-openai.js
var OPENAI_URL = "https://api.openai.com/v1/chat/completions";
var BRAND_PROVIDER = "SKYES OVER LONDON";
var BRAND_MODEL = "kAIxU";
var DEFAULT_MODEL = process.env.OPENAI_MODEL || "gpt-4.1-mini";
var FALLBACK_MODEL = process.env.OPENAI_FALLBACK_MODEL || "gpt-4o-mini";
var KAIXU_CORS = {
...
function normalizeMessages(payloadOrMessages) {
  if (Array.isArray(payloadOrMessages)) {
    const normalized = payloadOrMessages.filter(Boolean).map((m2) => ({
      role: ["system", "developer", "user", "assistant", "tool"].includes(m2.role) ? m2.role : "user",
  ...
  const sys = payload.systemInstruction?.parts?.map((p2) => p2?.text || "").join("\n").trim();
  if (sys) out.push({ role: "developer", content: sys });
  const contents = Array.isArray(payload.contents) ? payload.contents : [];
  for (const item of contents) {
    const role = item?.role === "model" ? "assistant" : "user";
    const text = Array.isArray(item?.parts) ? item.parts.map((p2) => p2?.text || "").join("\n").trim() : "";
...
async function callKaixu(payload = {}) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) {
    const err2 = new Error("Missing OPENAI_API_KEY");
    err2.statusCode = 500;
    throw err2;
  }
  ...
  for (const model of modelCandidates(payload)) {
    const body = {
      ...
      max_tokens: typeof payload.max_tokens === "number" ? payload.max_tokens : void 0,
      stream: false
    };
    const { resp, text, data } = await invokeChat(apiKey, body);
    if (!resp.ok) {
      ...
      const err2 = new Error(data?.error?.message || text || `OpenAI HTTP ${resp.status}`);
      err2.statusCode = resp.status;
      throw err2;
    }
...
function sseResponse({ outputText, usage, brand, resolvedModel }, month = null) {
  const events = [
...
function decodeNetlifyContext(context) {
  try {
...
async function ensureMember(sql, user) {
  const email = String(user?.email || "").trim().toLowerCase();
  if (!email) {
  ...
  const defaultRequestCap = clampInt(process.env.DEFAULT_MONTHLY_REQUEST_CAP, 250, 1, 1e5);
  const defaultTokenCap = clampInt(process.env.DEFAULT_MONTHLY_TOKEN_CAP, 25e4, 5e3, 5e8);
...
function monthStart(date = /* @__PURE__ */ new Date()) {
  const year = date.getUTCFullYear();
  const month = date.getUTCMonth();
  return new Date(Date.UTC(year, month, 1, 0, 0, 0)).toISOString();
}
async function getMonthUsage(sql, memberId) {
...
async function recordUsage(sql, member, payload) {
...
function enforceCap(member, usageMonth) {
