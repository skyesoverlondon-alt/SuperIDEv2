  ...
  try {
    ...
    if (event.httpMethod === "POST") {
      ...
      for (const log of logs.slice(0, 100)) {
        const message = String(log.message || "").slice(0, 4e3);
        if (!message) continue;
        await sql`
          ...
          VALUES (
            ...
            ${String(user?.sub || log.identity_id || "").slice(0, 255) || null},
            ${String(user?.email || log.email || "").slice(0, 255) || null}
          )
        `;
        inserted += 1;
      }
    ...
    if (event.httpMethod === "GET") {
      ...
      const limit = Math.min(parseInt(params.limit || "200", 10) || 200, 1e3);
