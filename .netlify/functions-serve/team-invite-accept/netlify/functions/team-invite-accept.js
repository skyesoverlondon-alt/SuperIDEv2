...
// netlify/functions/team-invite-accept.ts
var team_invite_accept_exports = {};
__export(team_invite_accept_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(team_invite_accept_exports);
...
function sha256Hex(input) {
  return import_crypto3.default.createHash("sha256").update(input).digest("hex");
}
var handler = async (event) => {
  ...
  const tokenExpiresAt = new Date(Date.now() + 90 * 24 * 60 * 60 * 1e3).toISOString();
  await q(
...
//# sourceMappingURL=team-invite-accept.js.map
