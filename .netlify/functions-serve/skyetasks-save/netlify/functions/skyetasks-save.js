...
// netlify/functions/skyetasks-save.ts
var skyetasks_save_exports = {};
__export(skyetasks_save_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(skyetasks_save_exports);
...
var handler = async (event) => {
  ...
  const title = String(body.title || "SkyeTasks Board").trim();
  ...
  try {
    ...
    if (recordId) {
      saved = await q(
        `update app_records
         ...
         where id=$3 and org_id=$4 and app='SkyeTasks'
      ...
      if (!saved.rows.length) {
        const current = await q(
          `select id, updated_at from app_records where id=$1 and org_id=$2 and app='SkyeTasks' limit 1`,
          [recordId, u.org_id]
        );
        if (!current.rows.length) return json(404, { error: "SkyeTasks record not found." });
        return json(409, {
          error: "SkyeTasks conflict: record changed by another editor.",
          conflict: true,
      ...
      const existing = await q(
        `select id, updated_at
         from app_records
         where org_id=$1 and app='SkyeTasks' and ws_id=$2
      ...
      if (existing.rows.length) {
        if (!expectedUpdatedAt || String(existing.rows[0].updated_at) !== expectedUpdatedAt) {
          return json(409, {
            error: "SkyeTasks conflict: sync latest workspace model before saving.",
            conflict: true,
        ...
        saved = await q(
          `insert into app_records(org_id, ws_id, app, title, payload, created_by)
           values($1,$2,'SkyeTasks',$3,$4::jsonb,$5)
           returning id, updated_at`,
          [u.org_id, wsId, title, JSON.stringify(model), u.user_id]
        );
      }
    }
    await audit(u.email, u.org_id, wsId, "skyetasks.save.ok", {
    ...
    const msg = e?.message || "SkyeTasks save failed.";
    await audit(u.email, u.org_id, wsId, "skyetasks.save.failed", { error: msg });
...
//# sourceMappingURL=skyetasks-save.js.map
