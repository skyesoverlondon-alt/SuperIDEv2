...
var team_invite_exports = {};
__export(team_invite_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(team_invite_exports);
...
// netlify/functions/_shared/mailer.ts
var import_nodemailer = __toESM(require_nodemailer(), 1);
var EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
function resolveDefaultFrom(input) {
...
async function assertOrgSeatCapacity(orgId, seatsRequested = 1) {
  ...
  if (summary.seats_reserved + seatsRequested > summary.seat_limit) {
    const error = new Error("Seat limit reached.");
...
// netlify/functions/team-invite.ts
var VALID_ROLES = /* @__PURE__ */ new Set(["owner", "admin", "member", "viewer"]);
...
var handler = async (event) => {
  ...
  return json(200, {
    ...
    seat_summary: {
      ...
      seats_available: seatSummary.seat_limit == null ? null : Math.max(seatSummary.seat_limit - (seatSummary.seats_reserved + 1), 0)
    }
  });
};
