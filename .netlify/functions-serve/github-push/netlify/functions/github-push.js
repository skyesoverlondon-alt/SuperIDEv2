...
// netlify/functions/github-push.ts
var github_push_exports = {};
__export(github_push_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(github_push_exports);
...
var handler = async (event) => {
  ...
  if (!r.rows.length || !r.rows[0].github_repo || !r.rows[0].github_installation_id) {
    return json(400, { error: "GitHub App not connected." });
  }
  const repo = r.rows[0].github_repo;
  const branch = r.rows[0].github_branch || "main";
  const installation_id = Number(r.rows[0].github_installation_id);
  ...
  try {
    const out = await runnerCall("/v1/github/app/push", {
      user_id: u.user_id,
      org_id: u.org_id,
      ws_id,
      repo,
      ...
      message: message || `kAIxU Super IDE update (${(/* @__PURE__ */ new Date()).toISOString()})`
    });
    const payload = {
      ...out,
      sknore: {
...
//# sourceMappingURL=github-push.js.map
